...
// Sorts the linked list in ascending or descending order based on the `ascending` parameter
void SortLinkedList(LinkedList list, unsigned int ascending) {
    ...
    do {
        ...
        while (current->next != NULL) {
            if ((ascending && strcmp(current->payload, current->next->payload) > 0) ||
                (!ascending && strcmp(current->payload, current->next->payload) < 0)) {
...
// Moves the iterator to the next element and returns 0 if successful, 1 otherwise
...
// Moves the iterator to the previous element and returns 0 if successful, 1 otherwise
...
int LLIterDelete(LLIter iter) {
    ...
    else iter->list->head = to_delete->next;
    if (to_delete->next) to_delete->next->prev = to_delete->prev;
    else iter->list->tail = to_delete->prev;
    if (to_delete->next) iter->cur_node = to_delete->next;
    else iter->cur_node = to_delete->prev;
    DestroyLinkedListNode(to_delete);
    iter->list->num_elements--;
    return 0;
}

// Inserts a new node with the given payload before the current node the iterator points to
